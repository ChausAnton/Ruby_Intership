...
module Instructions
  def instructions(conn, template)
    materials = conn.exec('SELECT * FROM marketing_materials ORDER BY marketing_materials_type;')

    fixtures = []
    conn.exec('SELECT * FROM fixtures').each do |fixture|
